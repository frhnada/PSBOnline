...
<div class="content-layout"><div class="content-layout-row"><div class="layout-cell layout-item-0" style="width: 100%" >
   <?php $template = array( 'table_open'         => '<table id="tsorter" class="tablesorter" style="width: 100%;">',
      ...
      $this->table->set_heading('NO', 'No. Pend.', 'E-mail', 'Password', 'Nama Lengkap', 'Nama Panggilan', 'Jenis Kelamin', 'Tempat Lahir', 'Tgl. Lahir', 'Anak Ke', 'Jml Sdr Kandung', 'Jml Sdr Angkat', 'Anak Yatim Piatu', 'Bahasa', 'Alamat', 'No. telp', 'Alamat Sdr Solo', 'Gol. Darah', 'Penyakit Yg Pernah Diderita', 'Kelainan Jasmani', 'Asal SMA', 'Universitas', 'Fakultas', 'Jurusan');
	  $query = 'SELECT gel_id FROM gelombang ORDER BY gel_id DESC LIMIT 1';
      $gel = $this->db->query($query);
	  $query = $query->row_array();
	  $query = 'SELECT * FROM calonsantri WHERE gel_CalonSantri == '.$gel['gel_id'].'';
	  ...
	  foreach ($query as $key => $row){
         ...
         //$siswa_no_pendaftaran = $row['noPendaftaran'];
         ...
         $this->table->add_row( '<td style="text-align: center;">'.$no.'</td>',
            '<td style="text-align: center;">'.$row['noPendaftaran'].'</td>',
